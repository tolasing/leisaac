#!/usr/bin/env bash
# Launch Franka NutThread teleoperation (LeIsaac-Franka-NutThread-v0)
# Usage: bash launch_franka_industrial.sh [--teleop_device keyboard|gamepad] [extra args...]
...
exec "${ISAAC_PYTHON}" "${TELEOP_SCRIPT}" \
    --task LeIsaac-Franka-NutThread-v0 \
